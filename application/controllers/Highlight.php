...
class Highlight extends CI_Controller
{
    ...
    private function _init()
    {
        ...
        $this->output->set_title("Highlight");
    ...
    public function index()
    {
        ...
        // die();
        $data = array(
            'module_title' => 'Highlight',
            'header' => $this->cfg_header,
            'record' => $highlight,
            'controller' => 'highlight',
        );
        $this->load->view('admin/highlight/list_table',$data);
    }

    public function read($id) 
    {
        $row = $this->Highlight_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'thn2014' => $row->thn2014,
            ...
            $this->load->view('highlight/highlight_read', $data);
        } else {
    ...
    public function create() 
    {
        $data = array(
            'module_title' =>'Highlight',
            'button' => 'Create',
            'action' => site_url( "highlight/create_action"),
        ...
        $this->load->view('admin/highlight/highlight_form', $data);
    }
    ...
    public function create_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            // $this->Highlight_model->update($this->input->post('id_provinsi', TRUE), $data);
            $this->Highlight_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            ...
            $data = array(
                'module_title' =>'Highlight',
                'button' => 'Update',
                'action' => site_url("highlight/update_action"),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            $id = $this->update($this->input->post( 'id_highlight', TRUE));
            ...
            // $this->Highlight_model->update($this->input->post('id_provinsi', TRUE), $data);
            $this->Highlight_model->update($this->input->post('id_highlight', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Highlight_model->delete($id);
	...
	foreach ($this->Highlight_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'post_data' => $this->Highlight_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('highlight/highlight_doc',$data);
    }
